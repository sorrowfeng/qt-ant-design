...
void drawEmptyIllustration(QPainter* painter, const AntEmpty* empty, const QSize& logicalSize)
{
    const auto& token = antTheme->tokens();
    ...
    if (simple)
    {
        painter->scale(logicalSize.width() / 128.0, logicalSize.height() / 80.0);
    ...
    {
        painter->scale(logicalSize.width() / 184.0, logicalSize.height() / 152.0);
...
void AntEmptyStyle::drawEmpty(const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    ...
    if (empty->imageVisible())
    {
        ...
        else
        {
            ...
            drawEmptyIllustration(&pixmapPainter, empty, r.size());
