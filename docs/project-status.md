...
- Reworked the `AntWindow` outer shadow on the Windows 10 no-caption path to use a transparent software shadow host behind the main window, with a tighter 18px Win11-like feather, capped corner opacity, symmetric shadow pixels on all four sides, and geometry that tracks visible resizes consistently.
