...
> 归档说明：这份文件是历史问题记录，保留用于回归排查和旧修复溯源。当前新增问题优先使用 GitHub Issues 跟踪；完整归档副本见 `docs/archive/issue-log.md`。
...
- **解决**:新增关闭确认 API:`closeConfirmationEnabled`、`closeConfirmationTitle`、`closeConfirmationContent`、`closeConfirmationOkText`、`closeConfirmationCancelText`。默认不启用退出确认,业务通过 `setCloseConfirmationEnabled(true)` 开启后,`closeEvent()` 会弹出 `AntModal` 确认框。取消按钮只关闭 Modal 并保留窗口,确认按钮通过 `forceClose()` 跳过二次确认并真正关闭窗口;示例程序 smoke 自动退出也改用 `forceClose()`。
