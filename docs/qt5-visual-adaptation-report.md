...
| Analyze High DPI effects | `AntDesign::configureHighDpi()` is called before `QApplication`; `AntDesign::initialize()` auto-configures High DPI when it is called before `QApplication`; `TestAntHighDpiScaling_*` covers 1.0, 1.25, 1.5, and the initialize-default startup path; `TestAntQtVersionVisualParity_Scale_1_25` and `_1_5` smoke-render the full atlas at fractional scaling. | Verified |
...
| --- | --- | --- | --- | --- |
| `src/core/AntDesign.h`, `src/core/AntDesign.cpp` | Application startup / Windows High DPI | Qt5 needs High DPI attributes before `QApplication`; otherwise layouts, fonts, hit regions, grabs, and pixmaps can be interpreted in physical pixels and vary between 100%, 125%, and 150% displays. | Added `AntDesign::configureHighDpi()` to enable `AA_EnableHighDpiScaling`, `AA_UseHighDpiPixmaps`, and pass-through scale-factor rounding before app creation. `initialize()` now calls that setup automatically when no `QApplication` exists, returns before font/theme work, and still warns on Qt5 if the startup path was skipped after app creation. | Qt5-only application attributes are behind `#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)`; pass-through rounding is guarded by `QT_VERSION >= 5.14`. |
...
| `tests/CMakeLists.txt` | Test registry | New parity, High DPI, no-QSS, and real example traversal checks needed CTest entries. | Added `TestAntQtVersionVisualParity`, `TestAntQtVersionVisualParity_Scale_1_25`, `TestAntQtVersionVisualParity_Scale_1_5`, the `TestAntHighDpiScaling_*` scale entries plus `TestAntHighDpiScaling_InitializeDefault`, `TestAntNoStyleSheetUsage`, and `TestAntExamplePageTraversal`. | CMake uses existing Qt-major abstraction. |
...
| CTest registration | Both Qt6 and Qt5 build trees list `47` CTest entries, including the default visual parity test, two fractional-scale visual parity tests, the metric audit, High DPI scale tests, the initialize-default High DPI test, no-QSS guard, and example traversal. |
...
- Call `AntDesign::configureHighDpi()` before `QApplication` in consumer apps, especially on Qt5/Windows. `AntDesign::initialize()` will call it automatically only when no `QApplication` exists yet; calling it after app creation is intentionally warned because Qt cannot retroactively switch the process into the same logical scaling mode.
