...
- `CI definition`: the checked-in workflow covers Windows Qt 5 Debug static plus Qt 6 Debug static/Release shared and installed example/consumer checks, Linux Qt 6 Debug static with ASan+UBSan plus Qt 5 Release shared, and macOS Qt 6 Release shared. Non-sanitizer configurations install; sanitizer configurations build and test without installing. This describes the configured matrix only; no remote pass is inferred from the workflow file.
