...
| Icon | Pass | `build/icon-state-compare-light.png`, `build/icon-state-compare-dark.png` | Compared default 14px/22px outlined icons, primary color, disabled color, rotate, loading spin frame, filled, two-tone, and large upload states. `AntIconStyle` now renders LoadingOutlined, SettingFilled, CheckCircleTwoTone, and HeartTwoTone from official SVG paths and uses AntD two-tone secondary fills for the audited colors. |
