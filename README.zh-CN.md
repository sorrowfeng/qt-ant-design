...
- Windows 10 走无 native caption 的窗口样式，并使用 legacy rounded mask 与透明软件阴影宿主窗口，避免最大化/还原后露出原生标题栏按钮，同时让普通窗口在缩放前后都保持从窗口边缘直接外扩、更轻、更接近 Win11 且圆角更干净的四周阴影。
...
- `AntMessage` / `AntNotification`：带浮层阴影、进入/退出动效以及 Notification loading 进度倒计时的全局反馈组件
