...
- Windows 10 下 `AntWindow` 使用无 `WS_CAPTION` 的 native style，避免最大化/吸附还原后露出原生最小化/最大化/关闭按钮；非最大化时使用 legacy rounded mask 裁掉直角窗口边界，并用透明软件阴影宿主窗口绘制更收敛、接近 Win11、缩放前后一致的四周阴影；最大化 `WM_NCCALCSIZE` 不再重复扣除 resize border。
