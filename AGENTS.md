...
- Qt Layout 自适应策略补齐：`AntSelect` / `AntInputNumber` 采用横向可扩展、纵向固定的输入控件尺寸策略，`AntTypography` 支持 wordWrap/ellipsis 的 height-for-width 高度计算，`AntList` 采用横纵可扩展策略以便受限高度下使用内部滚动。
