...
- 测试状态：`32 / 32` CTest 目标在 Debug 下通过（最近一次全量验证：`2026-05-01`）
...
- **测试数量**：32 个 CTest 目标（31 个 QTest 可执行文件 + 1 个安装消费方 CMake 脚本测试）
- **覆盖组件**：82 个公开组件全部覆盖，内部 helper 随宿主组件测试
- **运行方式**：`ctest -C Debug --output-on-failure`
- **最近全量结果**：`32 / 32` 通过（Debug，2026-05-01）
...
├── TestAntVisualRegression.cpp # Token-color, semantic-state, navigation-indicator, and light/dark visual guards
