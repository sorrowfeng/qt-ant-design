...
- `AntSelect` and `AntInputNumber` use layout-friendly horizontal expansion with fixed control height; `AntTypography` supports height-for-width word wrapping; `AntList` expands in both directions so constrained Qt layouts use its internal scrolling instead of clipping.
